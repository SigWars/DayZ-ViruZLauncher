...
using ViruZLauncher.Launcher.Configuration;
using ViruZLauncher.Launcher.Handlers.Protocols;
using ViruZLauncher.Launcher.Utility;

namespace ViruZLauncher.Launcher.Handlers
{
	/// <summary>
	...
	internal sealed class LauncherHandler
	{
		...
		private const string LocalExecutableName = "%ViruZLauncherExecutable%";
		...
		/// </summary>
		private static readonly IViruZLauncherConfiguration Configuration = ConfigHandler.Instance.Configuration;

		/// <summary>
		/// Initializes a new instance of the <see cref="ViruZLauncher.Launcher.Handlers.LauncherHandler"/> class.
		/// </summary>
		...
		private static string GetUpdateScriptResourceName()
		{
			if (PlatformHelpers.IsRunningOnUnix())
			{
				return "ViruZLauncher.Launcher.Resources.ViruZLauncher_update.sh";
			}
			else
			{
				 return "ViruZLauncher.Launcher.Resources.ViruZLauncher_update.bat";
			}
		}
		...
		private static string GetUpdateScriptPath()
		{
			if (PlatformHelpers.IsRunningOnUnix())
			{
				return $@"{Path.GetTempPath()}ViruZLauncher_update.sh";
			}
			else
			{
				 return $@"{Path.GetTempPath()}ViruZLauncher_update.bat";
			}
		}
