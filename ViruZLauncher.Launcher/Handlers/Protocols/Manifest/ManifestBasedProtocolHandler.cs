...
using NGettext;
using NLog;
using ViruZLauncher.Common;
using ViruZLauncher.Common.Enums;
using ViruZLauncher.Common.Handlers;
using ViruZLauncher.Common.Handlers.Manifest;
using ViruZLauncher.Launcher.Services;
using ViruZLauncher.Launcher.Utility;

namespace ViruZLauncher.Launcher.Handlers.Protocols.Manifest
{
	/// <summary>
	...
	public abstract class ManifestBasedProtocolHandler : PatchProtocolHandler
	{
		...
		/// </summary>
		private static readonly ICatalog LocalizationCatalog = new Catalog("ViruZLauncher", "./Content/locale");
		...
		public override void UpdateModule(EModule module)
		{
			...
			switch (module)
			{
				case EModule.Launcher:
				{
					...
					manifest = this.FileManifestHandler.GetManifest(EManifestType.ViruZLauncher, false);
					oldManifest = this.FileManifestHandler.GetManifest(EManifestType.ViruZLauncher, true);
					break;
				}
				case EModule.Game:
				{
		...
		{
			RefreshModuleManifest(EModule.Game); // SigWar Added module refresh on click Repair button to prevent bugs
		...
		protected override void DownloadModule(EModule module)
		{
			...
			switch (module)
			{
				case EModule.Launcher:
				{
					...
					moduleManifest = this.FileManifestHandler.GetManifest(EManifestType.ViruZLauncher, false);
					break;
				}
				case EModule.Game:
				{
